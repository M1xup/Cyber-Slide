...
public class DualHooks : MonoBehaviour
{
    ...
    [SerializeField] private EndRunnerGame _endRunnerGame;
    ...
    public KeyCode grappleKey = KeyCode.V;
    ...
    [Header("AudioEffects")]
    public AudioSource[] _hooksShotSources;
    ...
    private void Update()
    {
        ...
        if (swingingCdTimer > 0)
            swingingCdTimer -= Time.deltaTime;
    }
    ...
    private void StartSwing(int swingIndex)
    {
        ...
        if (_endRunnerGame != null && _pauseRunner != null && _learningMenu != null)
        {
            if (_endRunnerGame._endGame || _pauseRunner.Paused || _learningMenu.IsLearningMenu) return;
        }

        if (swingingCdTimer > 0) return;
    ...
    #region  HooksShotAudio

    private void HooksShotAudio()
    {
        var audioIndex = (int)Random.Range(0, _hooksShotSources.Length);
        _hooksShotSources[audioIndex].pitch = Random.Range(0.8f, 1.5f);
        _hooksShotSources[audioIndex].volume = Random.Range(0.7f, 1.0f);
        _hooksShotSources[audioIndex].Play();
    }
